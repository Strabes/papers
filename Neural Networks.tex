...
One complication that arises in training deep neural networks is the fact that the distribution of the inputs to each layer change during training as parameters from previous layers are updated. This change is known as internal covariate shift. \cite{ioffe2015batch} introduced Batch Normalization to normalize layers' inputs during mini-batch training. There is more than one way to normalize; in the discussion below I deviate slightly from \cite{ioffe2015batch}.
...
Layer normalization was introduced by \cite{ba2016layer}
...
\bibliography{neuralnetworks} % Entries are in the refs.bib file
