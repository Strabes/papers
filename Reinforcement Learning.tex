...
Reinforcement Learning is an area of machine learning that seeks to train models to take actions in a dynamic, stochastic environment in order to maximize a cumulative expected reward.
...
\subsection{Basic Policy Gradient Method}

All policy gradient methods use a version of the same two-step procedure:

\begin{enumerate}
\item{Estimate the gradient of the policy function}
\item{Use the estimated gradient in a stochastic gradient ascent algorithm}
\end{enumerate}

The estimate of the gradient typically takes the form

\begin{equation}
\hat{g} = \mathbb{\hat{E}}_t \left[ \triangledown_{\theta} \mathrm{ln} \pi_{\theta} \left(s_t, a_t, \theta \right) \hat{A}_t \right]
\end{equation}

\subsection{Trust Region Policy Optimization} 
 
Trust Region Policy Optimization was introduced by \cite{trpo}. TRPO optimizes

\begin{equation}
\mathrm{maximize}_{\theta} \mathbb{\hat{E}}_t \left[ 
\frac{\pi_{\theta} \left(s_t, a_t, \theta \right)}{\pi_{\theta_{old}} \left(s_t, a_t, \theta \right)}
 \hat{A}_t \right]
\end{equation}

\subsection{Proximal Policy Optimization}
Proximal Policy Optimization was introduced in \cite{schulmanppo}
